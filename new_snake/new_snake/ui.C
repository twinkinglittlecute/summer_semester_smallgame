...
#define FOOD_PIXEL2         30//new
#define FOOD_PIXEL3         40//new

// 用来绘图的颜色
#define COLOR_SNAKE			RGB(151, 255, 255)
#define COLOR_FOOD			RGB(153, 255, 51)
#define COLOR_DRUG          RGB(153, 50, 204)//毒药的颜色
#define COLOR_BOUNDARY		RGB(255, 250, 205)
#define COLOR_TEXT			RGB(173,216,230)

// 游戏的参数的设置 
#define INIT_TIMER_ELAPSE	350	// 初始的时钟周期，确定游戏初始速度
#define	ONE_LEVELS_SCORES	5	// 每升级一次需要的计分
#define INIT_SNAKE_LEN		5	// 蛇的长度
#define SPEEDUP_RATIO		0.80// 升级以后时间周期（确定游戏速度）提高的比例。* INIT_TIMER_ELAPSE   <1变快    >1变慢
#define MAX_X		35	// 游戏界面大小
#define MAX_Y		20	// 游戏界面大小
#define INIT_X		10	// 蛇的初始位置
#define INIT_Y		10	// 蛇的初始位置
	...
	int nCmdShow)   // 用于指明窗口是否需要显示的参数。
{
	...
	*******************************************************************************/
	...
	wc.lpfnWndProc = MainWndProc;//关键   zbj
	...
	hwnd = CreateWindow(
		...
		0,			// 窗口水平位置default horizontal POINT 
		0,			// 窗口垂直位置default vertical POINT 
	...
	*******************************************************************************/
		...
		&& fGotMessage != -1)
	{
		...
		DispatchMessage(&msg);//调用消息处理函数    zbjk
	}
	return msg.wParam;
}
//之前全部是局部变量的定义
void GamePaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	PGAME_COORD lpFood[2];
	PGAME_COORD lpDrug[18];
	int i, snake_size;

	GetClientRect(hwnd, &rect);

	hdc = GetDC(hwnd);//得到窗口的画图区域，这是一个物理的DC，在这上面画图就直接显示在上面了
	//容量和性能是不能兼顾的
	//看文档!!!!!
	...
	hdcmem = CreateCompatibleDC(hdc);//创建了一个与窗口兼容的虚拟DC，不能直接显示到界面上，能以位图形式拼接出来，按显示器固定的频率出现能提速
	...
	hbrushDrug = CreateSolidBrush(COLOR_DRUG);//毒药RGB颜色，实心BRUSH
	...
	*
	*******************************************************************************/
	FillRect(hdcmem, &rect, (HBRUSH)GetStockObject(WHITE_BRUSH));//直接获取白色画刷，系统一创建，在stock中可以得到
	...
	*******************************************************************************/
	...
	hOldBrush = (HBRUSH)SelectObject(hdcmem, hbrushFood);//画食物的话笔和画刷
	hOldPen = (HPEN)SelectObject(hdcmem, hpen);
	
	
		lpFood[0] = GetFood1();
		// 矩形，使用上面选择的PEN勾勒边框，BRUSH填充
		lpFood[1] = GetFood2();

		for (i = 0; i < 2; i++)
		{
			Ellipse(hdcmem,
				lpFood[i]->x * CELL_PIXEL + rectBoundary.left,
				lpFood[i]->y * CELL_PIXEL + rectBoundary.top,
				(lpFood[i]->x + 1)*CELL_PIXEL + rectBoundary.left,
				(lpFood[i]->y + 1)*CELL_PIXEL + rectBoundary.top);
		}
		/*******************************************************************************
		* #############  画毒药  ################
		*
		*******************************************************************************/
		hOldBrush = (HBRUSH)SelectObject(hdcmem, hbrushDrug);//画毒药的画笔和画刷
		hOldPen = (HPEN)SelectObject(hdcmem, hpen);
		...
		{
			Ellipse(hdcmem,
				lpDrug[i]->x * CELL_PIXEL + rectBoundary.left,
				lpDrug[i]->y * CELL_PIXEL + rectBoundary.top,
				(lpDrug[i]->x + 1)*CELL_PIXEL + rectBoundary.left,
				(lpDrug[i]->y + 1)*CELL_PIXEL + rectBoundary.top);
		}
		...
		*******************************************************************************/
		...
		hFont = CreateFont(48, 0, 0, 0, FW_DONTCARE, 0, 1, 0, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
			CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Consolas"));//比较复杂，用系统已有的字体，创建字体对象，放入DC中

		// 将这个FONT对象放入DC中
		if (hOldFont = (HFONT)SelectObject(hdcmem, hFont))
		{
			CHAR szSourceInfo[1024];//局部变量使用的内存空间是已经弄好了的
			wsprintf(szSourceInfo, "score %d level %d", GetScore(), GetLevel());//可以用text out不支持格式化输出，不输出到界面，先存在内存
			// 设置输出颜色
			SetTextColor(hdcmem, COLOR_TEXT);//设置字体颜色
			// 输出字符串。
			TextOut(hdcmem, (int)rectBoundary.left * 20, rectBoundary.bottom + 10,
		...
		BitBlt(hdc,
			...
			hdcmem, 0, 0, SRCCOPY);//内存拷贝，拷贝时还可以进行缩放，可以考全部或局部
	...
	LPARAM lParam) // 消息参数，不同的消息有不同的意义，详见MSDN中每个消息的文档
{
	...
	case WM_CREATE://程序收到的第一个消息   zbjk
		CreateGame(hwnd,//老师自己写的  zbjk
			INIT_TIMER_ELAPSE,
			ONE_LEVELS_SCORES,
			SPEEDUP_RATIO,
			MAX_X, MAX_Y,
	...
	case WM_PAINT://绘制界面   zbjk

		GamePaint(hwnd);
		break;

	case WM_KEYDOWN://处理键盘    zbjk
	...
	case WM_TIMER://计时器消息到达时，处理计时器消息并重回窗口

		OnTimer(hwnd);
		GamePaint(hwnd);
		break;

	case WM_DESTROY://退出整个游戏，相当于点了×
		ExitProcess(0);//销毁进程   zbj
		break;
